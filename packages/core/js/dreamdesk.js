...
function E(n, t) {
  ...
  e.onfinish = () => {
    n.style.position = "fixed", n.style.top = "0px", n.style.left = "0px", n.style.width = "100vw", n.style.height = "100vh", n.style.zIndex = "9999";
  };
}
function S(n, t) {
  y(n);
  ...
  e.onfinish = () => {
    n.style.position = t.position || "absolute", n.style.top = `${Math.round(t.top)}px`, n.style.left = `${Math.round(t.left)}px`, n.style.width = `${Math.round(t.width)}px`, n.style.height = `${Math.round(t.height)}px`, t.zIndex ? n.style.zIndex = t.zIndex : n.style.removeProperty("z-index"), y(n);
  };
}
function R(n, t) {
  n.animate(
...
}, $ = import.meta.url, T = $.slice(0, $.lastIndexOf("/") + 1), I = `${T}../css/`, P = 1e3, A = /* @__PURE__ */ new Map(), f = [];
function k() {
  f.forEach((n, t) => {
    const e = A.get(n);
...
class D extends v {
  ...
  minimize() {
    ...
    typeof s == "function" ? Promise.resolve(s(t, { defaultFns: { minimize: C }, previousState: this.state.previousState })).then(i) : (C(t), i());
  }
  fullscreen() {
    var a, o;
  ...
  _setupResizeObserver() {
    ...
    const t = new ResizeObserver((o) => {
      ...
      this._observedScrollables.forEach((l) => t.unobserve(l)), this._observedScrollables = [], s.forEach((l) => {
        var g, d;
        const u = (g = l.matches) != null && g.call(l, o) ? [l] : [], b = ((d = l.querySelectorAll) == null ? void 0 : d.call(l, o)) ?? [];
        [...u, ...Array.from(b)].forEach((c) => {
          c.classList.forEach((h) => {
            h.endsWith("-scroll") && c.classList.remove(h);
  ...
  _setupResizeHandle() {
    ...
    const l = 180, u = 120, b = (c) => {
      s && (this.style.setProperty("--ddw-w", `${Math.max(l, a + c.clientX - i)}px`), this.style.setProperty("--ddw-h", `${Math.max(u, o + c.clientY - r)}px`), this.setAttribute("data-ddw-explicit", ""));
    }, g = () => {
      s = !1, document.removeEventListener("pointermove", b, { capture: !0 }), document.removeEventListener("pointerup", g, { capture: !0 });
    };
    e.addEventListener("pointerdown", (c) => {
      var p, z, x;
      if ((p = this.state) != null && p.isFullscreen) return;
      s = !0, i = c.clientX, r = c.clientY;
      const h = t.getBoundingClientRect();
      a = h.width, o = h.height, document.addEventListener("pointermove", b, { capture: !0, signal: ((z = this._eventController) == null ? void 0 : z.signal) ?? void 0 }), document.addEventListener("pointerup", g, { capture: !0, signal: ((x = this._eventController) == null ? void 0 : x.signal) ?? void 0 });
  ...
  _setupDragging() {
    ...
    if (!t) return;
    if (!this._movable) {
      t.style.cursor = "default", (g = this._dragController) == null || g.abort(), this._dragController = null;
      return;
    }
    ...
    const u = (d) => {
      if (!s) return;
      const c = d.clientX - i, h = d.clientY - r;
      a && cancelAnimationFrame(a), a = requestAnimationFrame(() => {
        this.style.left = `${Math.max(0, Math.min(c, o))}px`, this.style.top = `${Math.max(0, Math.min(h, l))}px`;
      });
    }, b = () => {
      s = !1, document.removeEventListener("pointermove", u, { capture: !0 }), document.removeEventListener("pointerup", b, { capture: !0 });
    };
    t.addEventListener("pointerdown", (d) => {
      var h;
      if (!this._movable || (h = this.state) != null && h.isFullscreen && this.getAttribute("fullscreen-mode") !== "expand" || d.target.closest(".win-controls")) return;
      y(this);
      const c = this.getBoundingClientRect();
      i = d.clientX - c.left, r = d.clientY - c.top, o = Math.max(0, window.innerWidth - c.width), l = Math.max(0, window.innerHeight - c.height), s = !0, this.setAttribute("data-ddw-explicit", ""), this.style.setProperty("--ddw-w", `${c.width}px`), this.style.setProperty("--ddw-h", `${c.height}px`), getComputedStyle(this).position === "static" && (this.style.position = "absolute"), L(this._winId), document.addEventListener("pointermove", u, { capture: !0, signal: e }), document.addEventListener("pointerup", b, { capture: !0, signal: e });
    }, { signal: e });
  }
  _bindFocusRaise() {
    var t, e;
  ...
  _applyControlIcons() {
    const t = this.shadowRoot, e = (s, i) => {
      ...
      const o = a.trim();
      let l = "";
      if (o.startsWith("<svg") ? l = M(o) : (b = window.DreamDeskIcons) != null && b[o] && (l = M(window.DreamDeskIcons[o])), !l) return;
...
class B extends v {
  ...
  themeChanged() {
    ...
    if (e && t && this._segments.length) {
      ...
      this._segments.forEach((a, o) => {
        const { width: l, marginRight: u } = getComputedStyle(a), b = parseFloat(l) + (parseFloat(u) || 0);
        s ? (a.style.backgroundImage = "var(--color-progress-gradient, none)", a.style.backgroundSize = `${r}px 100%`, a.style.backgroundPosition = `-${o * b}px 0`, a.style.backgroundRepeat = "no-repeat", a.style.backgroundColor = "transparent") : (a.style.backgroundImage = "none", a.style.backgroundColor = "var(--color-progress-segment, #a8edea)");
  ...
  _afterRender() {
    ...
    if (e) {
      const r = () => {
        const g = t.getBoundingClientRect().width, d = Math.floor((g + 1) / 11), c = d * 11 - 1;
        t.innerHTML = "", this._segments = [];
        for (let h = 0; h < d; h++) {
          const p = document.createElement("div");
          p.className = "progress-segment", p.style.cssText = `width:10px;height:20px;margin-right:${h < d - 1 ? 1 : 0}px`, s && (p.style.backgroundImage = "var(--color-progress-gradient, none)", p.style.backgroundSize = `${c}px 100%`, p.style.backgroundPosition = `-${h * 11}px 0`, p.style.backgroundRepeat = "no-repeat"), t.appendChild(p), this._segments.push(p);
        }
        this._updateProgress();
      };
...
const _ = class _ extends v {
  ...
  template() {
    const t = this.action ? `data-action="${m(this.action)}"` : "", e = this.action ? `aria-label="${m(this.action)}"` : "", s = this.getAttribute("disabled"), i = s !== null && s !== "false" && s !== "0" ? 'disabled aria-disabled="true"' : "";
    return `<button class="btn btn--${m(this.variant)}" ${t} ${e} ${i}><slot></slot></button>`;
  }
  connectedCallback() {
...
class j extends v {
  ...
  template() {
    return `${this._label ? `<label class="input-label" for="${m(this._inputId)}">${m(this._label)}</label>` : ""}
...
customElements.define("dreamdesk-tab", W);
customElements.define("dreamdesk-tab-panel", X);
customElements.define("dreamdesk-tabs", H);
customElements.define("dreamdesk-window", D);
customElements.define("dreamdesk-progress-bar", B);
